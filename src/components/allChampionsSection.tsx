...
export default function AllChampionsSection({
  ...
  return (
    <div className="flex flex-col justify-center items-center py-6 bg-neutral-800 rounded-md mt-10 xl:py-6 xl:px-16">
      <TitleConfig
      ...
      >
        {data.map((champion, index) => (
          <ChampionSquareCard data={champion} key={index} />
        ))}
      </div>
    </div>
  );
